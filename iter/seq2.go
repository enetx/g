...
// Example:
//
//	pairs := []KV[int, string]{{1, "a"}, {2, "b"}, {3, "c"}, {4, "d"}}
...
// SortBy2 sorts the sequence using the provided comparison function on KV pairs.
...
// Reduce2 reduces the Seq2 to a single KV pair.
// Returns false if the sequence is empty.
//
// Example:
//
//	s := iter.FromMap(map[int]int{1: 10, 2: 20})
//	result, ok := iter.Reduce2(s, func(a, b KV[int, int]) KV[int, int] {
//	  return KV[int, int]{a.K + b.K, a.V + b.V}
//	}) // KV{3, 30}, true
...
// Example:
//
//	pairs := []KV[int, string]{{1, "a"}, {2, "b"}, {3, "c"}}
//	s := iter.FromPairs(pairs)
