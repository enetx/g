...
func DuplicateTokenEx(
	hExistingToken syscall.Token,
	dwDesiredAccess uint32,
	lpTokenAttributes *syscall.SecurityAttributes,
	impersonationLevel uint32,
	tokenType TokenType,
	phNewToken *syscall.Token,
) (err error) {
...
func GetAdaptersAddresses(
	...
	flags uint32,
	reserved uintptr,
	adapterAddresses *IpAdapterAddresses,
	sizePointer *uint32,
) (errcode error) {
...
func NetShareDel(serverName *uint16, netName *uint16, reserved uint32) (neterr error) {
	r0, _, _ := syscall.Syscall(
		...
		3,
		uintptr(unsafe.Pointer(serverName)),
		uintptr(unsafe.Pointer(netName)),
		uintptr(reserved),
	)
	if r0 != 0 {
		neterr = syscall.Errno(r0)
...
func NetUserGetLocalGroups(
	...
	r0, _, _ := syscall.Syscall9(
		procNetUserGetLocalGroups.Addr(),
		8,
		uintptr(unsafe.Pointer(serverName)),
...
func WSASocket(
	...
	typ int32,
	protocol int32,
	protinfo *syscall.WSAProtocolInfo,
	group uint32,
	flags uint32,
	...
	r0, _, e1 := syscall.Syscall6(
		...
		uintptr(typ),
		uintptr(protocol),
		uintptr(unsafe.Pointer(protinfo)),
		uintptr(group),
		uintptr(flags),
	)
