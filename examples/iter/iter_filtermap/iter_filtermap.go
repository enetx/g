...
func main() {
	...
	// FilterMap can also change the element type: parse and filter in one
	// pass, String -> Int (invalid entries are dropped)
	SliceOf[String]("10", "oops", "20", "-", "30").
		Iter().
		FilterMap(func(s String) Option[Int] { return s.TryInt().Option() }).
		Collect().
