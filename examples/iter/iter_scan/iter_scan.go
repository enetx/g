...
func main() {
	...
	// Example 7: the accumulator type may differ from the element type —
	// cumulative lengths of words (String elements, Int state)
	SliceOf[String]("alpha", "beta", "gamma").
		Iter().
		Scan(Int(0), func(acc Int, w String) Int { return acc + w.Len() }).
		Collect().
		Println() // Slice[0, 5, 9, 14]
