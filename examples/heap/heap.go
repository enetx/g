...
func PerformanceComparison() {
	...
	fmt.Printf("Results identical: %v\n", result1.Collect(cmp.Cmp).Eq(result2.Collect(cmp.Cmp)))
...
// Example 12: Heap Equality
func HeapEquality() {
	fmt.Println("\n=== Heap Equality ===")

	// Two heaps holding the same elements, inserted in different order
	h1 := NewHeap(cmp.Cmp[int])
	h1.Push(3, 1, 2)

	h2 := NewHeap(cmp.Cmp[int])
	h2.Push(2, 3, 1)

	// Eq compares the sorted iteration order, not the internal layout,
	// so equivalent heaps built differently are equal
	fmt.Printf("h1.Eq(h2): %v\n", h1.Eq(h2)) // true
	fmt.Printf("h1.Ne(h2): %v\n", h1.Ne(h2)) // false

	// Diverge: one extra element makes them unequal
	h2.Push(4)
	fmt.Printf("After Push(4), h1.Eq(h2): %v\n", h1.Eq(h2)) // false
}
