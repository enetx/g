...
// callAPI simulates an API call with various outcomes
...
// saveToDatabase simulates saving response to a database
func saveToDatabase(resp Response) {
	log.Printf("[V] Saved to DB: ID=%d, Status=%d, Duration=%v, Data=%q",
		resp.ID, resp.Status, resp.Duration, resp.Data)
}

func main() {
	log.Println("Starting API processing with pool...")
	...
	log.Printf("Total tasks: %d", p.TotalTasks())
	...
	successful.Iter().ForEach(func(resp Response) {
		saveToDatabase(resp)
	})
