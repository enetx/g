...
func main() {
	...
	String("post-55").RxReplace(pattern, "${1}38").Println()
	// Output: post-38
	...
	String("post-not-found").RxFind(pattern).UnwrapOr("post-333").Println()
	// Output: post-333

	// Find all matches of the pattern in the string
	String("some post-55 not found post-31 post-22").RxFindAll(pattern).Unwrap().Println()
	// Output: Slice[post-55, post-31, post-22]

	// Find a specific number of matches of the pattern in the string
	String("some post-55 not found post-31 post-22").RxFindAllN(pattern, 2).Some().Println()
	// Output: Slice[post-55, post-31]

	// Get the starting indices of the first match of the pattern in the string
	String("post-55").RxIndex(pattern).Some().Println()
	// Output: Slice[0, 7]

	// Find the submatches of the first match of the pattern in the string
	String("some post-55 not found post-31").RxFindSubmatch(pattern).Some().Println()
	// Output: Slice[post-55, post-]

	// Find all submatches of the pattern in the string
	String("some post-55 not found post-31 post-22").RxFindAllSubmatch(pattern).Some().Println()
	...
	String("some post-55 not found post-31 post-22").RxFindAllSubmatchN(pattern, 2).Some().Println()
