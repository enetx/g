...
func main() {
	...
	g.NewFile("test.gob").Encode().Gob(g.SliceOf(1, 2, 3)).Unwrap()

	// Decode the Gob file into a new slice of integers
	var gobdata g.Slice[int]
	g.NewFile("test.gob").Decode().Gob(&gobdata)
	...
	g.NewFile("test.json").Encode().JSON(g.SliceOf(1, 2, 3)).Unwrap()

	// Decode the JSON file into a new slice of integers
	var jsondata g.Slice[int]
	g.NewFile("test.json").Decode().JSON(&jsondata)
